...
def predidct_age_gender_race(save_prediction_at, imgs_path='cropped_faces/'):
    ...
    model_fair_4.load_state_dict(torch.load('fair_face_models/fairface_alldata_4race_20191111.pt', map_location=device))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_faces_dir', dest='save_faces_dir', action='store', default="detected_faces_custom",
                        help='directory where the detected faces will be saved')
    parser.add_argument('--output_csv', dest='output_csv', action='store', default="test_outputs_custom.csv",
                        help='CSV file path where the annotations for the images will be saved.')
    dlib.DLIB_USE_CUDA = True
    print("using CUDA?: %s" % dlib.DLIB_USE_CUDA)
    args = parser.parse_args()

    SAVE_DETECTED_AT = args.save_faces_dir
